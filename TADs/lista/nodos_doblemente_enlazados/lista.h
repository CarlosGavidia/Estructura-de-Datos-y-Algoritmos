	...
	*/
	...
	void enredar(){
		/*if (!esVacia()){
			...
			Nodo *segundo=NULL;
			Nodo *tercero=NULL;
			while ((primero->_sig!=NULL &&ultimo->_ant!=NULL) &&((primero->_sig !=ultimo)&&(ultimo->_ant!=primero))){
				segundo = primero->_sig;
				tercero = ultimo->_ant;
			...
			/*if (longitud()/2==1){
				Nodo *medio = primero->_sig;
				ultimo->_sig=tercero;
			}
		ultimo->_sig=NULL;
		
		}*/
		Lista<char>::Iterator i1 = begin();
		Lista<char>:: Iterator  i2 =Iterator(_ult);
