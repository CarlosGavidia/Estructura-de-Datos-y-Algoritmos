...
class Arbin {
      ...
      ~Arbin() {
	   ...
	   //ejercicio 1
		...
		int talla(){
			int aux=0;
			int altura=0;
			tallaAux(_ra, aux, altura);
			return altura;
		}
		Lista<T> frontera(){
			Lista<T> resul;
			fronteraAux(_ra, resul);
			return resul;
		}
		void reflejo (Arbin <T>& resul){
			return reflejoAux(resul._ra);
		}
   ...
   private:
	 ...
	 //ejercicio 1 
	 int numNodosAux(Nodo *n) {
		if (n==NULL) return 0; 
		else{
			return 1+ numNodosAux(n->_iz)+numNodosAux(n->_dr);
		}
	}
	bool esHojaAux(Nodo *n)const{
		return n !=NULL && n->_iz==NULL && n ->_dr==NULL ;
	}
	int numHojasAux(Nodo *n){
		if (n==NULL) return 0;
		else if (esHojaAux(n)) return 1;
		else{
			return numHojasAux(n->_iz)+numHojasAux(n->_dr);
		}
	}
	void tallaAux(Nodo *n, int aux, int &altura) {

		if(n==NULL){
			if (aux > altura) altura = aux;
		}
		else {
			tallaAux(n->_iz, aux+1, altura);
			tallaAux(n->_dr, aux+1, altura);    
		}
	}
	void fronteraAux(Nodo *n,Lista<T>&resul){
		if(n!=NULL){
			if (esHojaAux(n)){
				resul.pon_final(n->_elem);
			}
			fronteraAux(n->_iz,resul);
			fronteraAux(n->_dr,resul);
		}
	}
	Nodo *reflejoAux(Nodo *n){
		if (n==NULL) return NULL;
		else{
			return Nodo(reflejoAux(n->_dr),n->_elem,reflejoAux(n->_iz));
		}
	}
