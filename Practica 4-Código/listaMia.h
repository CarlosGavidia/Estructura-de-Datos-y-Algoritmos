...
public:
	...
	*/
	...
	protected:
		...
		rConstIterator() : _act(NULL) {}
		rConstIterator(Nodo *act) : _act(act) {}
	...
	*/
	...
	void enredar(){
		Nodo *primero=_prim;
		Nodo *siguiente=_prim->_sig;

		Nodo *ultimo=_ult;
		Nodo *anterior=_ult->_ant;

		while (primero!=ultimo){
			primero->_sig=ultimo;
			ultimo->_ant=primero;
			if (anterior->_sig!=siguiente){
			...
			if (anterior->_sig!=siguiente){
				ultimo=anterior;
				anterior=ultimo->_ant;
				siguiente=primero->_sig;
			}
		}
		primero->_sig=NULL;
	}
	...
	/*void enredar(){
		if (esVacia())throw EListaVacia();
			...
			Nodo *primero=_prim;
			Nodo *ultimo=_ult;
			Nodo *siguiente =_prim;
			Nodo *anterior=_ult;
			while (siguiente!=anterior && ultimo->_sig!=primero){
				siguiente=primero->_sig;
				anterior=ultimo->_ant;
				primero->_sig=ultimo;
				ultimo->_ant=primero;
			...
			if (_numElems%2==0)//si es par el numero de nodos
				primero->_sig=NULL;
			else
				ultimo->_sig=NULL;
